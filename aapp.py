...
# 1. إعدادات الصفحة (بدون إخفاء Fork و GitHub بناءً على طلبك)
st.set_page_config(page_title="BDS System", layout="wide")

# 2. تهيئة الذاكرة لحفظ الحالة (Session State) لضمان التعبئة لمرة واحدة
...
# 3. القائمة الجانبية (اللغة والـ API) - تفتح من السايدبار المعتاد
with st.sidebar:
...
# قاموس النصوص (بدون كلمة بروفايل)
t = {
    "Arabic": {
...
with st.expander(t["lead_h"], expanded=not st.session_state.lead_done):
    if st.session_state.lead_done:
        ...
        with col3:
            st.selectbox("الأسلوب القيادي", ["توجيهي", "تشاركي", "تفوي